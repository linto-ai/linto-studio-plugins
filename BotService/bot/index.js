  ...
  async init() {
    ...
    try {
      ...
      this.cleanupStream = async () => {
        ...
        this.logger.info('Browser closed, stream destroyed');
      };
      ...
      stream = await getStream(this.page, {
        audio: true,
        video: false,
        mimeType: 'audio/webm;codecs=pcm',
        audioConstraints: { mandatory: { sampleRate: 16000, channelCount: 1 } },
        frameSize: 20,
      });
      this.logger.info('Audio stream started');

      stream.on('data', (chunk) => {
        this.emit('data', chunk, this.session.id, this.channel.id);
      });
