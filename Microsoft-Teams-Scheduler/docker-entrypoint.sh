...
function setup_user() {
    ...
    if [ "${DEVELOPMENT}" = "true" ]; then
        ...
        # Change ownership of msteams-scheduler directory excluding its node_modules
        if [ -d /usr/src/app/msteams-scheduler ]; then
            find /usr/src/app/msteams-scheduler -path /usr/src/app/msteams-scheduler/node_modules -prune -o -exec chown "$USER_NAME:$GROUP_NAME" {} \;
        fi
...
echo "RUNNING : $1"
cd /usr/src/app/msteams-scheduler
