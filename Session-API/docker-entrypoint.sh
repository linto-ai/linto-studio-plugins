...
function setup_user() {
    ...
    if [ "${DEVELOPMENT}" = "true" ]; then
        ...
        # Change ownership of sessionapi directory excluding its node_modules
        if [ -d /usr/src/app/sessionapi ]; then
            find /usr/src/app/sessionapi -path /usr/src/app/sessionapi/node_modules -prune -o -exec chown "$USER_NAME:$GROUP_NAME" {} \;
        fi
