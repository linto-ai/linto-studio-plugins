...
function setup_user() {
    ...
    if [ "${DEVELOPMENT}" = "true" ]; then
        ...
        # Change ownership of teamsappservice directory excluding its node_modules
        if [ -d /usr/src/app/teamsappservice ]; then
            find /usr/src/app/teamsappservice -path /usr/src/app/teamsappservice/node_modules -prune -o -exec chown "$USER_NAME:$GROUP_NAME" {} \;
        fi
...
echo "Starting TeamsAppService..."
echo "HTTP Port: ${TEAMSAPPSERVICE_HTTP_PORT:-8082}"
echo "Components: ${TEAMSAPPSERVICE_COMPONENTS:-MeetingRegistry,BrokerClient,WebServer,WebSocketServer}"

cd /usr/src/app/teamsappservice
