...
// --- Startup handshake hardening (see Transcriber/VOXTRAL-HANDSHAKE.md) ---
...
class OpenAIStreamingTranscriber extends EventEmitter {
    ...
    constructor(session, channel) {
        ...
        // Startup handshake state (see VOXTRAL-HANDSHAKE.md).
    ...
    _onSessionCreated(config) {
        ...
        if (this.protocolName === 'vllm') {
            ...
            // running server-side (see VOXTRAL-HANDSHAKE.md).
            this._tryArm();
        } else {
     ...
     * the window: the server-side session is silently broken (see
     * VOXTRAL-HANDSHAKE.md). Reconnect; the pre-arm buffer carries the last
     * seconds of audio so the restart has no gap.
     */
