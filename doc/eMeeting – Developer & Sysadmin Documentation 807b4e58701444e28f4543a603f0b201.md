...
- **Session API Service:** A dedicated **Session API** is deployed as part of the live plugins. This service allows clients (like the Studio frontend or a mobile app) to create and manage live transcription sessions. A “session” represents a live meeting or stream that is being transcribed in real-time. Through the Session API, users can start a session (possibly specifying language or participants), stop a session, pause/resume a session (via `PUT /v1/sessions/:id/pause` and `PUT /v1/sessions/:id/resume`, which suspend ASR while keeping the audio stream alive), and fetch live transcription data. Sessions transition between the following statuses: `on_schedule` → `ready` → `active` ↔ `paused` → `terminated`, with a `pausedAt` timestamp tracked on the resource. Pause/resume transitions are also broadcast on MQTT via `system/out/sessions/paused` and `system/out/sessions/resumed`. Under the hood, starting a session will initiate the streaming pipeline (connecting to audio source, feeding STT continuously, etc.). This service also manages the state of live sessions and may coordinate multiple streams (audio, video) if needed. (Its API is documented via Swagger at `/session-api/api-docs/` for easy exploration)
