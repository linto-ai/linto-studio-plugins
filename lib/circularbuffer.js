class CircularBuffer {
    ...
    add(packet) {
      ...
      if (packetSize > remainingSpace) {
        ...
        const firstPart = packet.subarray(0, remainingSpace);
        const secondPart = packet.subarray(remainingSpace);
        this.buffer.set(firstPart, this.pointer);
        this.buffer.set(secondPart, 0);
        this.pointer = secondPart.length;
      } else {
