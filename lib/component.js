...
const { componentMissingError } = require(`./customErrors.js`)
...
class Component extends EventEmitter {
    ...
    async loadEventControllers(controllersDir) {
        console.log('loading controllers from', controllersDir)
        try {
            ...
            for (let item of currentDir) {
                ...
                } else if (item.toLocaleLowerCase().endsWith('.js')) {
    ...
    async init() {
        return new Promise(async (resolve, reject) => {
            try {
                await this.loadEventControllers(path.join(path.dirname(require.main.filename), "components", this.constructor.name, "/controllers"));
                resolve(this)
            } catch (e) {
                reject(e)
            }
