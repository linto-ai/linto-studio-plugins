...
class Component extends EventEmitter {
    ...
    async init() {
        return new Promise(async (resolve, reject) => {
            try {
                const controllersDir = path.join(path.dirname(require.main.filename), "components", this.constructor.name, "/controllers");
                await this.loadEventControllers(controllersDir);
                resolve(this)
            } catch (e) {
                if (e.code === 'ENOENT') {
                    resolve(this)
