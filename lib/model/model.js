...
const Session = sequelize.define('session', {
    ...
    // Private session id: the only id accepted on a stream (SRT streamid, RTMP
    // path, WS URL). Never returned by the API except inside the channels'
    // streamEndpoints. The public `id` stays the key for URLs, topics and events.
    privateId: {
        type: DataTypes.UUID,
        defaultValue: Sequelize.UUIDV4,
        allowNull: false,
    ...
    status: {
        type: DataTypes.ENUM('on_schedule', 'ready', 'active', 'paused', 'terminated'),
        allowNull: false
    },
    name: {
        type: DataTypes.STRING,
...
Channel.getPaginatedCaptions = async function(channelId, { limit = 50, offset = 0 } = {}) {
    const [totalClosedCaptions, [tcCount]] = await Promise.all([
        Caption.count({ where: { channelId } }),
        sequelize.query(
...
const Bot = sequelize.define('bot', {
    ...
    provider: {
        // 'visio' = LinTO Meet (LiveKit SFU) web bot, 'teams' = Microsoft Teams web bot.
        // Keep in sync with migration 20260622120000-add-visio-teams-bot-provider.
        type: DataTypes.ENUM('jitsi', 'bigbluebutton', 'teams', 'visio'),
        allowNull: false
    },
    url: {
        type: DataTypes.STRING,
    ...
    // Owning BotService replica uniqueId — set by the Scheduler when the bot is
    // scheduled, so a stopbot can be routed (and orphans reaped) across restarts.
    botservice: {
        type: DataTypes.STRING,
        allowNull: true
    }
