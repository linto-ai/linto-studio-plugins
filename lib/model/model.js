...
Channel.getPaginatedCaptions = async function(channelId, { limit = 50, offset = 0 } = {}) {
    const [result] = await sequelize.query(`
        SELECT
            jsonb_array_length(COALESCE(c."closedCaptions"::jsonb, '[]'::jsonb)) AS "totalClosedCaptions",
            COALESCE(
                (SELECT count(*)::int FROM jsonb_object_keys(
                    COALESCE(c."translatedCaptions"::jsonb, '{}'::jsonb)
                ) AS k),
                0
            ) AS "totalTranslatedCaptions",
            COALESCE(paginated.cc, '[]'::jsonb) AS "closedCaptions",
            COALESCE(
                (SELECT jsonb_object_agg(kv.key, kv.value)
                 FROM jsonb_each(COALESCE(c."translatedCaptions"::jsonb, '{}'::jsonb)) AS kv(key, value)
                 WHERE kv.key IN (
                     SELECT elem->>'segmentId'
                     FROM jsonb_array_elements(COALESCE(paginated.cc, '[]'::jsonb)) AS elem
                 )),
                '{}'::jsonb
            ) AS "translatedCaptions"
        FROM channels c
        LEFT JOIN LATERAL (
            SELECT jsonb_agg(elem ORDER BY ord ASC) AS cc
            FROM (
                SELECT elem, ord
                FROM jsonb_array_elements(COALESCE(c."closedCaptions"::jsonb, '[]'::jsonb))
