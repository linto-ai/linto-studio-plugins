...
const Bot = sequelize.define('bot', {
    ...
    provider: {
        type: DataTypes.ENUM('jitsi', 'bigbluebutton', 'teams', 'livekit'),
        allowNull: false
    },
    url: {
        type: DataTypes.STRING,
...
// TEAMS ACCOUNT LINKS (Azure user ↔ emeeting org mapping)
const TeamsAccountLink = sequelize.define('teamsAccountLink', {
    id: {
        type: DataTypes.UUID,
        defaultValue: Sequelize.UUIDV4,
        primaryKey: true
...
const CalendarSubscription = sequelize.define('calendarSubscription', {
    id: {
        type: DataTypes.UUID,
        defaultValue: Sequelize.UUIDV4,
        primaryKey: true
...
CalendarSubscription.belongsTo(TranscriberProfile, { foreignKey: 'transcriberProfileId' });
TranscriberProfile.hasMany(CalendarSubscription, { foreignKey: 'transcriberProfileId' });
CalendarSubscription.hasMany(MsTeamsEvent, { foreignKey: 'calendarSubscriptionId' });
MsTeamsEvent.belongsTo(CalendarSubscription, { foreignKey: 'calendarSubscriptionId' });
...
// MEDIA HOSTS
const MediaHost = sequelize.define('mediaHost', {
    id: {
        type: DataTypes.UUID,
        defaultValue: Sequelize.UUIDV4,
        primaryKey: true
...
IntegrationConfig.hasMany(MediaHost, { foreignKey: 'integrationConfigId' });
MediaHost.belongsTo(IntegrationConfig, { foreignKey: 'integrationConfigId' });
IntegrationConfig.belongsTo(MediaHost, { as: 'sharedMediaHost', foreignKey: 'sharedMediaHostId' });
