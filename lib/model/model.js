...
Channel.getPaginatedCaptions = async function(channelId, { limit = 50, offset = 0 } = {}) {
    const [result] = await sequelize.query(`
        SELECT
            jsonb_array_length(COALESCE("closedCaptions"::jsonb, '[]'::jsonb)) AS "totalClosedCaptions",
            jsonb_array_length(COALESCE("translatedCaptions"::jsonb, '[]'::jsonb)) AS "totalTranslatedCaptions",
            COALESCE(
                (SELECT jsonb_agg(elem ORDER BY ord ASC) FROM (
                    SELECT elem, ord FROM jsonb_array_elements(COALESCE("closedCaptions"::jsonb, '[]'::jsonb))
            ...
            COALESCE(
                (SELECT jsonb_agg(elem ORDER BY ord ASC) FROM (
                    SELECT elem, ord FROM jsonb_array_elements(COALESCE("translatedCaptions"::jsonb, '[]'::jsonb))
                        WITH ORDINALITY AS t(elem, ord)
                    ORDER BY ord DESC LIMIT :limit OFFSET :offset
                ) sub),
                '[]'::jsonb
