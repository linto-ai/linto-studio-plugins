...
class Security {
  constructor() {
    this.keyEnv = process.env.SECURITY_CRYPT_KEY;
    this.saltPath = process.env.SECURITY_SALT_FILEPATH || '';
    if (!this.keyEnv) {
      throw new Error('The environment variable SECURITY_CRYPT_KEY is required.');
    }
  }
