...
function setup_user() {
    ...
    if [ "${DEVELOPMENT}" = "true" ]; then
        ...
        # Change ownership of migration directory excluding its node_modules
        if [ -d /usr/src/app/migration ]; then
            find /usr/src/app/migration -path /usr/src/app/migration/node_modules -prune -o -exec chown "$USER_NAME:$GROUP_NAME" {} \;
        fi
