    ...
    for (const channel of channels) {
      ...
      for (let i = 0; i < rows.length; i += BATCH_SIZE) {
        ...
        const values = batch.map(row => {
          ...
          const segmentId = e.segmentId != null ? parseInt(e.segmentId, 10) : 'NULL';
          const start = e.start != null ? parseFloat(e.start) : 'NULL';
          const end = e.end != null ? parseFloat(e.end) : 'NULL';
    ...
    for (const channel of translatedChannels) {
      ...
      for (let i = 0; i < rows.length; i += BATCH_SIZE) {
        ...
        const values = batch.map(row => {
          const e = typeof row.elem === 'string' ? JSON.parse(row.elem) : row.elem;
          const channelId = channel.id;
          const segmentId = parseInt(row.segment_id, 10);
