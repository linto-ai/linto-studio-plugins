...
- Duration: ~10 s (kept short so scenarios stay fast).
- Provenance: derived from the repository's `en.mp3` (English speech sample at
  the repo root), trimmed to the first 10 seconds and transcoded with:

  ```bash
  ffmpeg -y -i en.mp3 -t 10 -ar 16000 -ac 1 -c:a pcm_s16le \
